...
function DitheredWaves({
  ...
  // Memoize uniforms so they're not recreated on every render
  const waveUniformsRef = useMemo<React.MutableRefObject<WaveUniforms>>(
    () => ({
      current: {
  ...
  }, [size.width, size.height, gl, waveUniformsRef]);

  // Memoize color to avoid unnecessary allocations
  const waveColorRef = useRef(new THREE.Color(...waveColor));
  useEffect(() => {
    waveColorRef.current.set(...waveColor);
  }, [waveColor]);

  // Optimize useFrame: only update values, not objects
  useFrame(({ clock }) => {
    if (!disableAnimation) {
    ...
    waveUniformsRef.current.waveColor.value.copy(waveColorRef.current);
  ...
  // Use ref for mouse position, avoid state update
  ...
  // Memoize RetroEffect
  const MemoizedRetroEffect = useMemo(
  ...
  return (
    <>
      ...
      <EffectComposer>{MemoizedRetroEffect}</EffectComposer>
    </>
  );
}
...
export default function Dither({
  ...
  const ditheredWavesProps = useMemo(
    () => ({
      waveSpeed,
      waveFrequency,
      waveAmplitude,
  ...
  return (
    ...
    >
      <DitheredWaves {...ditheredWavesProps} />
    </Canvas>
  );
}
